pipeline {
  ...
  stages {
    ...
    stage('Start Resource-Server for Performance and Integration Testing'){
      steps{
        script{
          sh 'scp Jmeter/ResourceServer.jmx jenkins@jenkins-master:/var/lib/jenkins/iudx/acl-apd/Jmeter/'
          sh 'scp src/test/resources/IUDX-Resource-Server-Consumer-APIs-V4.5.0.postman_collection.json jenkins@jenkins-master:/var/lib/jenkins/iudx/acl-apd/Newman/'
    ...
    stage('Jmeter Performance Test'){
      steps{
        ...
        node('built-in') {
          script{
            sh 'rm -rf /var/lib/jenkins/iudx/acl-apd/Jmeter/report ; mkdir -p /var/lib/jenkins/iudx/acl-apd/Jmeter/report'
            sh "set +x;/var/lib/jenkins/apache-jmeter/bin/jmeter.sh -n -t /var/lib/jenkins/iudx/acl-apd/Jmeter/ResourceServer.jmx -l /var/lib/jenkins/iudx/acl-apd/Jmeter/report/JmeterTest.jtl -e -o /var/lib/jenkins/iudx/acl-apd/Jmeter/report/ -Jhost=jenkins-slave1 -JpuneToken=$env.puneToken -JsuratToken=$env.suratToken"
          }
          perfReport filterRegex: '', showTrendGraphs: true, sourceDataFiles: '/var/lib/jenkins/iudx/acl-apd/Jmeter/report/*.jtl'
        }
      }
      post{
        failure{
      ...
      steps{
        node('built-in') {
          script{
            ...
            sh 'HTTP_PROXY=\'127.0.0.1:8090\' newman run /var/lib/jenkins/iudx/acl-apd/Newman/IUDX-Resource-Server-Consumer-APIs-V4.5.0.postman_collection.json -e /home/ubuntu/configs/acl-apd-postman-env.json -n 2 --insecure -r htmlextra --reporter-htmlextra-export /var/lib/jenkins/iudx/acl-apd/Newman/report/report.html --reporter-htmlextra-skipSensitiveData'
            runZapAttack()
          }
        }
      }
    ...
    stage('Continuous Deployment') {
      ...
      stages {
        ...
        stage('Docker Swarm deployment') {
          steps {
            script {
              sh "ssh azureuser@docker-swarm 'docker service update acl-apd --image ghcr.io/datakaveri/acl-apd-depl:5.0.0-alpha-${env.GIT_HASH}'"
              sh 'sleep 10'
            }
          }
          post{
        ...
        stage('Integration test on swarm deployment') {
          steps {
            node('built-in') {
              script{
                sh 'newman run /var/lib/jenkins/iudx/acl-apd/Newman/IUDX-Resource-Server-Consumer-APIs-V4.5.0.postman_collection.json -e /home/ubuntu/configs/cd/acl-apd-postman-env.json --insecure -r htmlextra --reporter-htmlextra-export /var/lib/jenkins/iudx/acl-apd/Newman/report/cd-report.html --reporter-htmlextra-skipSensitiveData'
              }
            }
          }
          post{
            always{
              node('built-in') {
                script{
                  publishHTML([allowMissing: false, alwaysLinkToLastBuild: true, keepAll: true, reportDir: '/var/lib/jenkins/iudx/acl-apd/Newman/report/', reportFiles: 'cd-report.html', reportTitles: '', reportName: 'Docker-Swarm Integration Test Report'])
                }
              }
            }
            failure{
  ...
  post{
    failure{
      script{
        ...
        emailext recipientProviders: [buildUser(), developers()], to: '$ACL_APD_RECIPIENTS, $DEFAULT_RECIPIENTS', subject: '$PROJECT_NAME - Build # $BUILD_NUMBER - $BUILD_STATUS!', body: '''$PROJECT_NAME - Build # $BUILD_NUMBER - $BUILD_STATUS:
