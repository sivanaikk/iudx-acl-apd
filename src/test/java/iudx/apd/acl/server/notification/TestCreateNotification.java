...
public class TestCreateNotification {
    ...
    @Mock
    static Future<List<ResourceObj>> resourceInfo;
    ...
    public void testInitiateCreateNotification(VertxTestContext vertxTestContext) {
        ...
        when(emailNotification.sendEmail(any(User.class), any(User.class), anyString(), anyString())).thenReturn(Future.succeededFuture(true));
        ...
        createNotification.initiateCreateNotification(notification, consumer).onComplete(handler -> {
            if (handler.succeeded()) {
                ...
                utility.executeQuery(Tuple.tuple(), "SELECT * FROM request").onComplete(selectQueryHandler -> {

                    JsonObject response = selectQueryHandler.result().getJsonArray("response").getJsonObject(1);
                    assertEquals(utility.getOwnerId().toString(), response.getString("owner_id"));
                    assertEquals(itemId.toString(), response.getString("item_id"));
                    assertEquals(TYPE_RESOURCE, response.getString("item_type"));
                    assertEquals(utility.getConsumerId().toString(), response.getString("user_id"));
    ...
    public void testWithNotificationAlreadyCreated(VertxTestContext vertxTestContext) {
        ...
        createNotification.initiateCreateNotification(notification, consumer).onComplete(handler -> {
            if (handler.succeeded()) {
                ...
                assertEquals(ResponseUrn.SUCCESS_URN.getMessage(), handler.result().getString(TITLE));
                assertEquals("Request inserted successfully", handler.result().getString(RESULT));
                createNotification.initiateCreateNotification(notification, consumer).onComplete(createNotificationHandler -> {
                    if (createNotificationHandler.succeeded()) {
                        vertxTestContext.failNow("Succeeded when notification is already created");
                    } else {
                        JsonObject failureMessage = new JsonObject()
                                .put(TYPE, 409)
                                .put(TITLE, POLICY_ALREADY_EXIST_URN.getUrn())
                                .put(DETAIL, "Request could not be created, as a request for the given resource has been previously made");
                        assertEquals(failureMessage.encode(), createNotificationHandler.cause().getMessage());
                        vertxTestContext.completeNow();
                    }
                });
    ...
    @Test
    @DisplayName("Test initiateCreateNotification when the catalogue returns failed response: Failure")
    public void testWithFailedResponseFromCat(VertxTestContext vertxTestContext) {
        ...
        createNotification.initiateCreateNotification(notification, consumer).onComplete(handler -> {
            if (handler.succeeded()) {
                vertxTestContext.failNow("Succeeded when the response it failed to get response from catalogue" );
            } else {
                JsonObject failureMessage = new JsonObject()
                        .put(TYPE, 404)
                        .put(TITLE, ResponseUrn.RESOURCE_NOT_FOUND_URN.getUrn())
                        .put(DETAIL, "Request could not be created, as resource was not found in the catalogue");
    ...
    public void testCreateNotificationWithEmptyResponse(VertxTestContext vertxTestContext)
    {
        ...
        createNotification.createNotification(CREATE_NOTIFICATION_QUERY, resourceId, TYPE_RESOURCE_GROUP, consumer, utility.getOwnerId()).onComplete(handler -> {
            if (handler.succeeded()) {
                ...
                JsonObject failureMessage = new JsonObject(handler.cause().getMessage());
                assertEquals(500, failureMessage.getInteger(TYPE));
                assertEquals(ResponseUrn.INTERNAL_SERVER_ERROR.getUrn(), failureMessage.getString(TITLE));
