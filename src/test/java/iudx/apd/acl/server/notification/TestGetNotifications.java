...
public class TestGetNotifications {
    ...
    public static User getOwner() {
        JsonObject jsonObject = new JsonObject().put("userId", utility.getOwnerId()).put("userRole", "provider").put("emailId", utility.getOwnerEmailId()).put("firstName", utility.getOwnerFirstName()).put("lastName", utility.getOwnerLastName());
        return new User(jsonObject);
    }

    public static User getConsumer() {
        JsonObject jsonObject = new JsonObject().put("userId", utility.getConsumerId()).put("userRole", "consumer").put("emailId", utility.getConsumerEmailId()).put("firstName", utility.getConsumerFirstName()).put("lastName", utility.getConsumerLastName());
        return new User(jsonObject);
    }
    ...
    public void testGetNotificationSuccess(VertxTestContext vertxTestContext) {
        getNotification.initiateGetNotifications(consumer).onComplete(handler -> {
            if (handler.succeeded()) {
                ...
                assertEquals(utility.getConsumerFirstName(), result.getJsonObject("consumer").getJsonObject("name").getString("firstName"));
                assertEquals(utility.getConsumerLastName(), result.getJsonObject("consumer").getJsonObject("name").getString("lastName"));
                assertEquals(utility.getConsumerEmailId(), result.getJsonObject("consumer").getString("email"));

                assertEquals(utility.getOwnerId().toString(), result.getJsonObject("provider").getString("id"));
                assertEquals(utility.getOwnerFirstName(), result.getJsonObject("provider").getJsonObject("name").getString("firstName"));
                assertEquals(utility.getOwnerLastName(), result.getJsonObject("provider").getJsonObject("name").getString("lastName"));
                assertEquals(utility.getOwnerEmailId(), result.getJsonObject("provider").getString("email"));

                assertEquals(HttpStatusCode.SUCCESS.getValue(), handler.result().getInteger(STATUS_CODE).intValue());
                assertEquals(ResponseUrn.SUCCESS_URN.getUrn(), handler.result().getJsonObject(RESULT).getString(TYPE));
                assertEquals(ResponseUrn.SUCCESS_URN.getMessage(), handler.result().getJsonObject(RESULT).getString(TITLE));
    ...
    public void testGetNotification4ProviderSuccess(VertxTestContext vertxTestContext) {
        getNotification.initiateGetNotifications(owner).onComplete(handler -> {
            if (handler.succeeded()) {
                JsonObject result = handler.result().getJsonObject(RESULT).getJsonArray(RESULT).getJsonObject(0);
                assertEquals(utility.getRequestId().toString(), result.getString("requestId"));
                assertEquals(utility.getResourceId().toString(), result.getString("itemId"));
    ...
    public void testExecuteGetNotification(VertxTestContext vertxTestContext) {
        JsonObject jsonObject = new JsonObject().put("userId", UUID.randomUUID()).put("userRole", "consumer").put("emailId", utility.getConsumerEmailId()).put("firstName", utility.getConsumerFirstName()).put("lastName", utility.getConsumerLastName());
        getNotification.initiateGetNotifications(new User(jsonObject)).onComplete(handler -> {
            if (handler.succeeded()) {
    ...
    public void testExecuteGetNotificationFailure(VertxTestContext vertxTestContext) {
        ...
        getNotification.initiateGetNotifications(consumer).onComplete(handler -> {
            if (handler.succeeded()) {
                ...
                JsonObject failureMessage = new JsonObject().put(TYPE, HttpStatusCode.INTERNAL_SERVER_ERROR.getValue()).put(TITLE, ResponseUrn.DB_ERROR_URN.getUrn()).put(DETAIL, "Notifications could not be fetched, Failure while executing query");
                assertEquals(failureMessage.encode(), handler.cause().getMessage());
                vertxTestContext.completeNow();
            }
        });
    ...
    public void testExecuteGetNotificationWithNullPgPool(VertxTestContext vertxTestContext) {
        ...
        GetNotification getNotification = new GetNotification(pgService);
        assertThrows(NullPointerException.class, () -> getNotification.initiateGetNotifications(consumer));
        vertxTestContext.completeNow();
    }
    ...
    public void testGetUserNotificationWithNullUser(VertxTestContext vertxTestContext) {
        ...
        assertThrows(NullPointerException.class, () -> getNotification.getUserNotification(null, "SELECT * FROM policy", Role.CONSUMER));
        vertxTestContext.completeNow();
    }
