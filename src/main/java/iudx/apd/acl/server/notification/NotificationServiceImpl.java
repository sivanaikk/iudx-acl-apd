...
public class NotificationServiceImpl implements NotificationService {
  ...
  public Future<JsonObject> createNotification(JsonObject request, User user) {
    ...
    createNotification
        .initiateCreateNotification(request, user)
        .onComplete(
            handler -> {
  ...
  public Future<JsonObject> deleteNotification(JsonObject notification, User user) {
    Promise<JsonObject> promise = Promise.promise();
    deleteNotification
        .initiateDeleteNotification(notification, user)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
  ...
  public Future<JsonObject> updateNotification(JsonObject request, User user) {
    Promise<JsonObject> promise = Promise.promise();
    updateNotification
        .initiateUpdateNotification(request, user)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
